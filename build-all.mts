...
// Determine asset path prefix based on BASE_URL
// For local development, use /assets/ prefix
// For external storage (like Supabase), use direct path
const isExternalStorage = baseUrlRaw !== defaultBaseUrl && 
  (baseUrlRaw.startsWith("http://") || baseUrlRaw.startsWith("https://"));

// Normalize base URL: ensure it ends with / for external storage, remove trailing slashes for local
let normalizedBaseUrl: string;
let assetPathPrefix: string;

if (isExternalStorage) {
  // For external storage, ensure base URL ends with / and use direct path
  normalizedBaseUrl = baseUrlRaw.replace(/\/+$/, "") + "/";
  assetPathPrefix = "";
} else {
  // For local development, remove trailing slashes and use /assets/ prefix
  normalizedBaseUrl = baseUrlRaw.replace(/\/+$/, "") || defaultBaseUrl;
  assetPathPrefix = "/assets/";
}

console.log(`Using BASE_URL ${normalizedBaseUrl} for generated HTML`);
console.log(`Asset path prefix: ${assetPathPrefix || "(none - direct from base URL)"}`);
...
<html>
<head>
 <script type="module" src="${normalizedBaseUrl}${assetPathPrefix}${name}-${h}.js"></script>
 <link rel="stylesheet" href="${normalizedBaseUrl}${assetPathPrefix}${name}-${h}.css">
</head>
<body>
